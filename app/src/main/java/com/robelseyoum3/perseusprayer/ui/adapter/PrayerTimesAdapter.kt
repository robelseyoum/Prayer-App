...
class PrayerTimesAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private fun updatePrayerTime(holder: PrayerTimeVH, position: Int) {
        when (position) {
            0 -> {
                val timeLineView =
                    TimelineView(holder.itemView.context, R.layout.view_time_line_top)
            ...
            list.size - 1 -> {
                val timeLineView =
                    TimelineView(holder.itemView.context, R.layout.view_time_line_bottom)
                holder.timeLineContainer.addView(timeLineView)
                holder.name.text = "Isha"
                holder.time.text = "Isha time"
